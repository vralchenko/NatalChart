...
export const BirthDataForm: React.FC<Props> = ({ onSubmit, loading, title = 'Birth Data' }) => {
  const [date, setDate] = useState<Dayjs | null>(null);
  const [time, setTime] = useState<Dayjs | null>(null);
  const [location, setLocation] = useState<GeocodingResult | null>(null);
  const [houseSystem, setHouseSystem] = useState(0); // Placidus
  ...
  const isValid = date?.isValid() && time?.isValid() && location;
          ...
          <form onSubmit={handleSubmit}>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                ...
                >
                  {loading ? <CircularProgress size={24} sx={{ color: 'white' }} /> : 'Calculate Chart'}
                </Button>
              </Grid>
            </Grid>
          </form>
